...
''' Esta função recebe uma string s, substitui todas as aspas (") contidas na string
pela sequência \" e devolve a nova string s.                                     '''
...
def main ():
    file = open ("constroi_banco.sql", "x")
    ...
    book = openpyxl.load_workbook ("[CEPI Fake News 2018] Base de URLs_v.2021.xlsx")
    sheet = book["URLs"]
    create_table (sheet, file)
    write_insertions (sheet, file)
    file.write ("\n")
